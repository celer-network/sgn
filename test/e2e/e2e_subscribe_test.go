...
func subscribeTest(t *testing.T) {
	...
	tf.ChkErr(err, "failed to query request on sgn")
	log.Infoln("Query sgn about the request info:", request.String())
	rstr := fmt.Sprintf(`SeqNum: %d, PeerAddresses: \[0x%s 0x%s\], PeerFromIndex: %d, SignedSimplexStateBytes: %x, TriggerTxHash: 0x[a-z0-9]{64}, GuardTxHash: 0x[a-z0-9]{64}, GuardEthAddress:`, 10, client0AddrStr, client1AddrStr, 0, signedSimplexStateBytes)
