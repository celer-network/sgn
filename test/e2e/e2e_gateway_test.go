...
func gatewayTest(t *testing.T) {
	...
	_, err = http.Post("127.0.0.1:1317/subscribe/subscribe", "application/json", bytes.NewBuffer(body))
	tf.ChkErr(err, "failed to post subscribe msg")

	resp, err := http.Get("127.0.0.1:1317/subscribe/subscription/" + ethAddress.String())
