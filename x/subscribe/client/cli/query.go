...
// Query request info
func QueryRequest(cliCtx context.CLIContext, queryRoute string, channelId []byte, peerTo string) (request types.Request, err error) {
	data, err := cliCtx.Codec.MarshalJSON(types.NewQueryRequestParams(channelId, peerTo))
	if err != nil {
		return
	}
