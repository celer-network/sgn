...
// Query request info
func QueryRequest(cliCtx context.CLIContext, queryRoute string, channelId []byte, owner string) (request types.Request, err error) {
	data, err := cliCtx.Codec.MarshalJSON(types.NewQueryRequestParams(channelId, owner))
	if err != nil {
		return
	}
