...
type SimpleTestIterator struct {
	Event *SimpleTest // Event containing the contract specifics and raw log
...
func (it *SimpleTestIterator) Next() bool {
...
func (it *SimpleTestIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *SimpleTestIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// SimpleTest represents a Test event raised by the Simple contract.
...
//
// Solidity: event Test(uint256 i)
func (_Simple *SimpleFilterer) FilterTest(opts *bind.FilterOpts) (*SimpleTestIterator, error) {

	logs, sub, err := _Simple.contract.FilterLogs(opts, "Test")
	if err != nil {
		return nil, err
	}
	return &SimpleTestIterator{contract: _Simple.contract, event: "Test", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event Test(uint256 i)
func (_Simple *SimpleFilterer) WatchTest(opts *bind.WatchOpts, sink chan<- *SimpleTest) (event.Subscription, error) {

	logs, sub, err := _Simple.contract.WatchLogs(opts, "Test")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				...
				event := new(SimpleTest)
				if err := _Simple.contract.UnpackLog(event, "Test", log); err != nil {
					return err
				}
				event.Raw = log
